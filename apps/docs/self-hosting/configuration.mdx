...
Public file URLs are always built as `http(s)://<S3_ENDPOINT>[:<S3_PORT>]/<S3_BUCKET>/<key>`, so they point at the same bucket files are uploaded to. `S3_PUBLIC_CUSTOM_DOMAIN` is only used to recognize older file URLs when deleting them.
...
The [Webhook block](/editor/blocks/logic/webhook) pauses a conversation until an
external service calls its callback URL. That endpoint is guarded by a single
shared secret.

| Parameter     | Default                | Description                                                                                                                  |
| ------------- | ---------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| WEBHOOK_TOKEN | `Bearer typebot_12345` | Expected `Authorization` header on webhook callbacks. **Change this**: the default is public and it guards every bot you host. |
