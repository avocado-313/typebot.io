...
This video provides a step-by-step guide to successfully configure HTTP request blocks in Azeer.
...
Feel free to reach out to support for help if you struggle setting up a Webhook block.
...
The Webhook block request fail or didn't seem to trigger? Make sure to check the [logs](/results/overview#logs). If you still can't figure out what went wrong, reach out to support.

### Webhook is not executed in the right order?

That is expected, Azeer actually computes all the blocks between each input blocks from a server before displaying anything.
