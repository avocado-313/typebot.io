...
With the OpenAI block, you can create a chat completion based on your user queries and display the answer back to your bot.
...
`Create Chat Message` and `Ask Assistant` blocks support vision. This means that Azeer automatically detects images URL in any user message provided to OpenAI and parse it. The URL needs to be isolated from the rest of the text message to be properly detected. Here is an example of a message with an image URL:
